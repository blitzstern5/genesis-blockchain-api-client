...
str="(cd $proj_dir && PYTHONPATH=$proj_dir $python_bin $this_dir/update_sys_params.py --priv-key=$priv_key --api-url=$api_url --name=new_version_url --value=some.url.site --name=max_block_generation_time --value=4000 --name=gap_between_blocks --value=3)"
eval "$str"
